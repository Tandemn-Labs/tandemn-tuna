...
class AzureProvider(InferenceProvider):
    ...
    def deploy(self, plan: ProviderPlan) -> DeploymentResult:
        ...
        # 1. Create Managed Environment with GPU workload profile
        logger.info("Creating Container Apps environment: %s in %s", env_name, region)
        ...
        container_app = aca_models.ContainerApp(
            location=region,
            managed_environment_id=f"/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.App/managedEnvironments/{env_name}",
        ...
        # Extract FQDN
        fqdn = result_app.configuration.ingress.fqdn if result_app.configuration and result_app.configuration.ingress else None
        endpoint_url = f"https://{fqdn}" if fqdn else None

        if not request_is_public(plan):
            logger.warning(
    ...
    def destroy(self, result: DeploymentResult) -> None:
        ...
        # Delete Managed Environment (may fail if other apps exist)
        if env_name:
            logger.info("Deleting Container Apps environment: %s", env_name)
            try:
                poller = client.managed_environments.begin_delete(resource_group, env_name)
                poller.result()
            except Exception as e:
                logger.warning("Failed to delete environment %s: %s (may have other apps)", env_name, e)
...
def request_is_public(plan: ProviderPlan) -> bool:
    """Check if the deployment was requested as public."""
    return plan.metadata.get("public_access") == "true"
